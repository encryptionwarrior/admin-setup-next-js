...
export default function page() {
  ...
  const initType = (searchParams.get("type") as AppType) || "all"
  const initFilter = searchParams.get("filter") || ""

  const [sort, setSort] = useState<"asc" | "desc">(initSort)
  const [appType, setAppType] = useState<AppType>(initType)
  const [searchTerm, setSearchTerm] = useState(initFilter)

  // Update URL when state changes
  useEffect(() => {
    const params = new URLSearchParams()
    if (searchTerm) params.set("filter", searchTerm)
    if (appType !== "all") params.set("type", appType)
    if (sort !== "asc") params.set("sort", sort)
    router.replace(`${pathname}?${params.toString()}`)
  }, [sort, appType, searchTerm, router, pathname])
